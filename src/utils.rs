...
pub fn ui_rect_clamp(a:Rect,b:Rect) -> Rect {
    Rect { min:a.min.clamp(b.min, b.max), max:a.max.clamp(b.min, b.max)}
}
pub fn ui_rect_expand(a:Rect,b:Rect)->Rect {
    Rect {
        min:a.min - b.min,
        max :a.max + b.max,
    }
}
pub fn ui_rect_is_zero(a:Rect) -> bool {
    a==Rect::default()
}
