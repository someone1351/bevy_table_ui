                ...
                for child_entity in children.clone() //.iter()
                {
                    ...
                    if let Ok(mut child_computed) = computed_query.get_mut(child_entity) { //todo replace with unwrap
                        ...
                        if child_computed.enabled {
                            ...
                            child_computed.border_edge=calc_edge(child_edge.border,child_computed.size.x,child_computed.size.y);
                            child_computed.padding_edge=calc_edge(child_edge.padding,child_computed.size.x,child_computed.size.y);
                            child_computed.margin_edge=calc_edge(child_edge.margin,child_computed.size.x,child_computed.size.y);
                ...
                for child_entity in children.clone() //.iter()
                {
                    ...
                    if let Ok(mut child_computed) = computed_query.get_mut(child_entity) {
                        if child_computed.enabled {
                            ...
                            let houter =
                                child_computed.border_edge.min.x+child_computed.border_edge.max.x+
                                child_computed.padding_edge.min.x+child_computed.padding_edge.max.x+
                                child_computed.margin_edge.min.x+child_computed.margin_edge.max.x;

                            let vouter =
                                child_computed.border_edge.min.y+child_computed.border_edge.max.y+
                                child_computed.padding_edge.min.y+child_computed.padding_edge.max.y+
                                child_computed.margin_edge.min.y+child_computed.margin_edge.max.y;
                            ...
                            if child_float || parent_of_root {
                                ...
                                child_computed.cell_edge.min.x=left_space;
                                child_computed.cell_edge.max.x=(hspace-left_space).max(0.0); //incase of floating point errors?
                                ...
                                child_computed.cell_edge.min.y=top_space; //ydir2
                                child_computed.cell_edge.max.y=(vspace-top_space).max(0.0); //ydir2
                            } else {
                                // let col = child_ind % cols_num;
                                // let row = child_ind / cols_num;
                                ...
                                child_computed.cell_edge.min.y=top_space;
                                child_computed.cell_edge.max.y=(vspace-top_space).max(0.0);
                ...
                for child_entity in children.clone() //.iter()
                {
                    ...
                    if child_computed.enabled && !child_float && !parent_of_root{
                        ...
                        let w=child_computed.size.x+
                            ...
                            child_computed.padding_edge.min.x+child_computed.padding_edge.max.x+
                            child_computed.margin_edge.min.x+child_computed.margin_edge.max.x+
                            child_computed.cell_edge.min.x+child_computed.cell_edge.max.x;

                        let h=child_computed.size.y+
                            child_computed.border_edge.min.y+child_computed.border_edge.max.y+
                            child_computed.padding_edge.min.y+child_computed.padding_edge.max.y+
                            child_computed.margin_edge.min.y+child_computed.margin_edge.max.y+
                            child_computed.cell_edge.min.y+child_computed.cell_edge.max.y;
                        ...
                        if child_computed.enabled {
                            ...
                            if child_float || parent_of_root {
                                ...
                                child_computed.pos.x=computed.pos.x+
                                    child_computed.cell_edge.min.x+
                                    child_computed.border_edge.min.x+
                                    child_computed.padding_edge.min.x+
                                    child_computed.margin_edge.min.x;

                                child_computed.pos.y=computed.pos.y+
                                    child_computed.cell_edge.min.y+
                                    child_computed.border_edge.min.y+
                                    child_computed.padding_edge.min.y+
                                    child_computed.margin_edge.min.y; //ydir2
                            } else {
                                // let col = child_ind % cols_num;
                                // let row = child_ind / cols_num;
    ...
    // for &entity in df_entities.iter()
    {
        ...
        if computed.enabled {
            ...
            let cell_rect = ui_rect_expand(inner_rect,
                ...
                Rect {
                    min: computed.padding_edge.min + computed.border_edge.min + computed.margin_edge.min + computed.cell_edge.min,
                    max: computed.padding_edge.max + computed.border_edge.max + computed.margin_edge.max + computed.cell_edge.max,
                }
            );
