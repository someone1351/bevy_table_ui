            ...
            for i in 0..focus_entity_stk.len() {
                ...
                if !root_entity_alive || !focusable_enabled || !unlocked || node_focus_group!=cur_group { 
                    ...
                    if let Some(entity)=*cur_focus_entity {
                        ui_event_writer.send(UiInteractEvent{entity,event_type:UiInteractEventType::FocusEnd{group:cur_group}});
    ...
    //handle focused==true, but not in focus_entity_stk/cur_focus_entity
    {
        for entity in focus_query.iter() {
            ...
            for i in focus_entity_stk.len() .. focus_ancestor_entities.len() {
                ...
                // prev_focused_stk.push(Default::default());
                ui_event_writer.send(UiInteractEvent{entity,event_type:UiInteractEventType::FocusBegin{group:cur_group}});
    ...
    for (&(root_entity,focus_group),&(entity,order)) in root_focus_group_focuses.iter() {
        ...
        if cur_focus_entity.is_none() && focus_entity_stk.is_empty() {
            ...
            for i in focus_entity_stk.len() .. focus_ancestor_entities.len() {
                let entity=focus_ancestor_entities[i];
                focus_entity_stk.push(entity);
                ui_event_writer.send(UiInteractEvent{entity,event_type:UiInteractEventType::FocusBegin{group:focus_group}});
        ...
        //
        {
            ...
            if let Some(cur_focus_entity)=*cur_focus_entity {
                ...
                for cur_entity in [cur_focus_entity].into_iter().chain(parent_query.iter_ancestors(cur_focus_entity)) {
                    let Ok(parent_entity) = parent_query.get(cur_entity).map(|p|p.get()) else {
            ...
            while let Some((entity, from_bounds, to_bound, focus_depth,valid))=stk.pop() {
                ...
                if let Some(focusable) = focusable_query.get(entity).ok() {
                    if cur_group==focusable.group && focusable.enable {
                        if let Some(cur_focus_entity) = *cur_focus_entity {
                            ui_event_writer.send(UiInteractEvent{entity:cur_focus_entity,event_type:UiInteractEventType::FocusEnd{group:cur_group}});
            ...
            //             ui_event_writer.send(UiEvent{entity,event_type:UiEventType::FocusPrev{group:cur_group,moved:found}});
            //         }
            //         UiInputEvent::FocusNext(..) => {
            //             ui_event_writer.send(UiEvent{entity,event_type:UiEventType::FocusNext{group:cur_group,moved:found}});
            //         }
            //         UiInputEvent::FocusLeft(..) => {
            //             ui_event_writer.send(UiEvent{entity,event_type:UiEventType::FocusLeft{group:cur_group,moved:found}});
            //         }
            //         UiInputEvent::FocusRight(..) => {
            //             ui_event_writer.send(UiEvent{entity,event_type:UiEventType::FocusRight{group:cur_group,moved:found}});
            //         }
            //         UiInputEvent::FocusUp(..) => {
            //             ui_event_writer.send(UiEvent{entity,event_type:UiEventType::FocusUp{group:cur_group,moved:found}});
            //         }
            //         UiInputEvent::FocusDown(..) => {
            //             ui_event_writer.send(UiEvent{entity,event_type:UiEventType::FocusDown{group:cur_group,moved:found}});
            //         }
            //         _ => {
            //         }
            //     }
